...
with tf.Session() as sess:
    # Run the initializer
    sess.run(init)
    summary = tf.summary.FileWriter("./graph1/",graph=tf.get_default_graph())
    # Training
    for i in range(1, num_steps + 1):
        ...
        train_acc, train_loss,train_sum = sess.run(
            [optimizer, loss,summaries],
            feed_dict={X: batch_x})
        # summary.add_summary(train_sum, i)
